...
router.get("/search/:name", async (request, response) => {
  ...
  const searchQueryResponse = await fetch(
    `https://www.alphavantage.co/query?function=SYMBOL_SEARCH&keywords=${request.params.name}&apikey=EU18KIHPYJ49OM7D`
  );
  ...
  const priceQueryResponse = await fetch(
    `https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol=${stockSymbolConst}&apikey=EU18KIHPYJ49OM7D`
  );
